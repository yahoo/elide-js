...
class JsonApiDatastore extends Datastore {
  ...
  find(query) {
    ...
    } else if (options && options.include instanceof Object) {
      ...
      Object.keys(query._opts.filters).forEach(function(modelName) {
        ...
        modelFilters.forEach((filterDef) => {
          if (typeof filterDef.operator === 'string') {
            queryParams[`filter[${modelName}.${filterDef.value}][${filterDef.operator}]`] = filterDef.value;
          } else {
            queryParams[`filter[${modelName}.${filterDef.value}]`] = filterDef.value;
          }
        });
      });
    }
