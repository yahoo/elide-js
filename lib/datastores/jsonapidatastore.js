...
class JsonApiDatastore extends Datastore {
  ...
  find(query) {
    ...
    var req = {path: url};
    request.appendQueryString(req);
    logUrl('GET', req.path);
    return request.then(this._transformResult.bind(this),
      (error) => {
        printDebug(error);
  ...
  create(model, toCreate) {
    ...
    let url = this._absoluteUrl(modelUrl);
    var request = SuperAgent.post(url, postData)
      .set(this._headers)
      .set('Content-Type', JSONAPI_MIME_TYPE);

    var req = {path: url};
    request.appendQueryString(req);
    logUrl('POST', req.path, JSON.stringify(postData, null, 2));
    return request.then(this._transformResult.bind(this),
      (error) => {
        printDebug(error);
  ...
  update(model, toUpdate) {
    ...
    let url = this._absoluteUrl(modelUrl);
    var request = SuperAgent.patch(url, patchData)
      .set(this._headers)
      .set('Content-Type', JSONAPI_MIME_TYPE);

    var req = {path: url};
    request.appendQueryString(req);
    logUrl('PATCH', req.path, JSON.stringify(patchData, null, 2));
    return request.then(this._transformResult.bind(this),
      (error) => {
        printDebug(error);
  ...
  delete(model, toDelete) {
    ...
    let url = this._absoluteUrl(modelUrl);
    var request = SuperAgent.del(this._absoluteUrl(modelUrl))
      .set(this._headers)
      .set('Content-Type', JSONAPI_MIME_TYPE);

    var req = {path: url};
    request.appendQueryString(req);
    logUrl('DELETE', req.path);
