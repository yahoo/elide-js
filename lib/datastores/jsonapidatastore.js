...
class JsonApiDatastore extends Datastore {
  ...
  _find(query) {
    ...
    if ('_opts' in query && query._opts instanceof Object) {
      if ('fields' in query._opts && query._opts.fields instanceof Object) {
        Object.keys(query._opts.fields).forEach(function(type) {
          urlParams['fields[' + type + ']'] = query._opts.fields[type].join();
        });
      } else {

        var fieldParams = new Array();
        var model = this._models[this._model];
        let linkNames = Object.keys(this._models[this._model].links || {});
        ...
        urlParams['fields[' + this._model + ']'] = fieldParams.join(",");
      }
    }
  ...
  _addRequestHeader(key, value) {
    ...
    _addFieldParameter(key, value) {
      this._computedFieldParams = '';
      this._fieldParameters[key] = value;
    }
